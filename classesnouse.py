...
class ChessPiece:
    def copy(self):
        return ChessPiece(self.type, self.pos, self.color, self.str.text)
    ...
    def __init__(self, type_, pos, color, symbol):
        self.color = color 
        self.pos = pos
        self.moves = [] #possivel posições na qual a peça pode se mover
        self.type = type_
    ...
    def move__ (self, ipos, fpos, piece_matrix):
        # if piece_matrix[ipos[0]][ipos[1]] != None:
        #     print(ipos)
        #     print(fpos)
        #     print(piece_matrix[ipos[0]][ipos[1]].moves)
        #     if not (fpos in piece_matrix[ipos[0]][ipos[1]].moves):
        #         return False
        piece_matrix[fpos[0]][fpos[1]] = piece_matrix[ipos[0]][ipos[1]]
        piece_matrix[fpos[0]][fpos[1]].change(fpos)
        piece_matrix[ipos[0]][ipos[1]] = None
        return True
    
    def update_poskings__(self, piece_matrix):
        posbking = None
        poswking = None
                ...
                if piece != None:
                    if piece.type == "King":
                        if piece.color == "White":
                            ...
                            posbking = piece.pos
        return poswking, posbking
        
    def up_move_chess__(self, initial_place, final_place, piece_matrix, color, move = True):
        if move:
            if not self.move__(initial_place, final_place, piece_matrix):
                return
        poswking, posbking = self.update_poskings__(piece_matrix)

        attackedplacesbyblack = []
        attackedplacesbywhite = []

        for row in piece_matrix:
            for piece in row:
                if piece != None:
                    if piece.type != "King":
                        piece.up_move(piece_matrix, color)
                        a = piece.moves
                        if piece.color == "Black":
                            attackedplacesbyblack.extend(a)
                        else:
                            attackedplacesbywhite.extend(a)
        if color == "White":
            return attackedplacesbyblack, poswking
        else:
            return attackedplacesbywhite, posbking

    def copy_matrix(self, piece_matrix):
        matrix = []
        for i in range(8):
            row = []
            for j in range(8):
                if piece_matrix[i][j] != None:
                    row.append(piece_matrix[i][j].copy())
                else:
                    row.append(None)
            matrix.append(row)
        return matrix

    def unprotect_king(self, piece_matrix_, color):
        for move in self.moves:
            print(self.moves)
            print(self.pos)
            print()            
    
            piece_matrix = self.copy_matrix(piece_matrix_)
            attackedplaces, posking = self.up_move_chess__(self.pos, move, piece_matrix, color)
            if posking in attackedplaces:
...
class Rook(ChessPiece):
    ...
    def up_move(self, piece_matrix, color):
        ...
        self.unprotect_king(piece_matrix, color)
...
class ChessGame:
    ...
    def move (self, ipos, fpos):
        ...
        if self.piece_matrix[ipos[0]][ipos[1]] == None:
            temp = True
        else:
    ...
    def initial_pieces(self):
        ...
        self.replace(King([0,4], "White"))
        self.replace(Rook([2, 4], "White"))
        self.replace(Rook([2, 5], "White"))
        self.replace(Rook([5, 4], "Black"))

    def update_pos_kings(self):
        for row in self.piece_matrix:
            for piece in row:
                if piece != None:
                    if piece.type == "King":
                        if piece.color == "White":
                            self.poswking = piece.pos
                        else:
                            self.posbking = piece.pos
    ...
    def up_move(self, initial_place, final_place, move = True):
        ...
        for row in self.piece_matrix:
            for piece in row:
                if piece != None:
                    if piece.type != "King": 
                        piece.up_move(self.piece_matrix, self.turn)
